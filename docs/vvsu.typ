...
let list-render(kind, level: 1, items) = {
  ...
  // Отступ всего списка в зависимости от уровня:
  // 1 уровень — 0 см
  // 2 уровень — 1,25 см
  // 3 уровень — 2,5 см
  let level-indent = (level - 1) * 1.25cm

  block(
    inset: (left: level-indent),
  )[
    #for (i, item) in items.children.enumerate() {
      let marker = if kind == "enum" {
        if type(items.numbering) == function {
          (items.numbering)(i + 1)
        } else {
          numbering(items.numbering, i + 1)
        }
      } else {
        [–]
      }
      ...
      let body = {
        ...
        show list: it => list-render(
          "list",
          level: level + 1,
          it,
        )

        show enum: it => list-render(
          "enum",
          level: level + 1,
          it,
        )
...
show enum: it => list-render("enum", it)
  ...
  show figure.where(kind: image): it => {
    ...
    context {
      ...
      _image-block(number, it.caption.body, it.body)
    }
  }
...
//// Титульный лист и индивидуальное задание для учебной практики
#let practice-title-page(
  student: [],
  group: [],
  supervisor1: [],
  supervisor2: [],
...
grid(
  columns: (130pt, 1fr, auto),
  column-gutter: 5pt,
  row-gutter: 10pt,
  align: bottom,

  [Студент\ гр. #group], line(length: 100%), student,
  [Руководитель,\ ассистент], line(length: 100%), supervisor1,
  [Руководитель,\ профессор], line(length: 100%), supervisor2,
)
  v(44pt)
  pagebreak(weak: true)
}
...
#let practice-assignment(
  ...
  [Студенту гр. #group #student]

  v(10pt)

  [1 Разработать программный продукт «#topic» (вариант #variant):]

  v(4pt)

  block(inset: (left: 40pt))[
    – Загрузка и объединение нескольких CSV-файлов с одинаковой структурой.\
    – Группировка записей по выбранному полю.\
    – Вычисление статистических характеристик: количества, суммы, среднего арифметического, медианы, минимального и максимального значений.\
    – Сортировка сформированных результатов.\
    – Формирование горизонтальной и процентной ASCII-диаграмм.\
    – Экспорт сформированного отчёта в форматы CSV и TXT.
  ]
  ...
  [#text(weight: "bold")[3 Срок сдачи отчёта на кафедру: #deadline]]
  ...
  grid(
    ...
    column-gutter: 5pt,
    align: bottom,
    [Задание получил:], line(length: 100%), student,
  )
  v(18pt)
